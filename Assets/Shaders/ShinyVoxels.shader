	...
	SubShader{
		...
		CGPROGRAM
			...
			inline fixed4 LightingTF2 (SurfaceOutput s, fixed3 lightDir, fixed3 viewDir, fixed atten)
			{
				...
				fixed4 c;
				c.rgb = ((s.Albedo * (ramp + 0.1f) * _LightColor0.rgb + (_LightColor0.rgb) * spec) * (atten * 2));
				c.a = s.Alpha;
				return c;
			}
			...
			void surf (Input IN, inout SurfaceOutput o)
			{
				...
				half3 rim = pow(max(0, dot(float3(0, 1, 0), WorldNormalVector(IN, o.Normal))), _RimPower) * _RimColor.rgb * _RimColor.a * specGloss.b - 0.15f;										
				o.Emission = rim;
			}
		ENDCG
	}
